...
import shutil
import subprocess
import sys  # using sys.platform is prohibited since this always runs on MacOS
...
def setup(tool_id, platform_, python_version_, abi_, dir_path, dest, project, version):
    ...
    if os.path.exists(pathlib.Path(dir_path) / "NionSwiftLauncher.exe"):
        shutil.move(pathlib.Path(dir_path) / "NionSwiftLauncher.exe", pathlib.Path(dir_path) / "NionSwift.exe")
...
installs = [
    ...
    ("nionswift", "macosx_10_11_intel", "cp35.cp36.cp37", "abi3", "NionSwiftLauncher-Mac", "bin", "nionswift-launcher", "0.3.19"),
    ("nionswift", "win_amd64", "cp35.cp36.cp37", "none", "NionSwiftLauncher-Windows", "Scripts/NionSwiftLauncher", "nionswift-launcher", "0.3.19"),
    ("nionswift", "manylinux1_x86_64", "cp35.cp36.cp37", "abi3", "NionSwiftLauncher-Linux", "bin/NionSwiftLauncher", "nionswift-launcher", "0.3.19"),
]
